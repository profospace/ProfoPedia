/**
 * Data utility functions for handling location data
 */

// Mapping of tehsil codes to SRO codes (in case they're different)
// In many cases they might be the same, but this allows for flexibility
export const tehsilToSroMap = {
    "208": "208", // Sadar First
    "209": "209", // Sadar Second
    "210": "210", // Sadar Third
    "211": "211", // Sadar Fourth
    "212": "212", // Bilhaur
    "213": "213", // Ghatampur
    "370": "370", // Narwal
...
/**
 * Load tehsil data for a given district
 * @param {string} districtCode - The district code
 * @returns {Promise<Array>} - Array of tehsil objects
 */
export const fetchTehsils = async (districtCode) => {
    // In a real implementation, this would be an API call
    // For demo purposes, we're returning static data

    // Sample tehsil data for Kanpur Nagar (164)
    if (districtCode === "164") {
        return [
    ...
    // Sample tehsil data for other districts
    // You would add actual data for other districts here

    return [];
};

/**
 * Load village data for a given tehsil
 * @param {string} tehsilCode - The tehsil code
 * @returns {Promise<Array>} - Array of village objects
 */
export const fetchVillages = async (tehsilCode) => {
    // In a real implementation, this would be an API call
    // For demo purposes, we're returning a subset of static data
...
/**
 * Search villages across all tehsils or within a specific tehsil
 * @param {string} query - The search query
 * @param {string} tehsilCode - Optional tehsil code to limit search scope
 * @returns {Promise<Array>} - Array of matching village objects
 */
export const searchVillages = async (query, tehsilCode = null) => {
    ...
    // If tehsil code is provided, only search within that tehsil
    if (tehsilCode) {
        const villages = await fetchVillages(tehsilCode);
        return villages.filter(village =>
            village.nameEn.toLowerCase().includes(normalizedQuery) ||
            village.name.includes(normalizedQuery)
        );
    }

    // Otherwise, search across all tehsils
    // This could be resource-intensive if there are many tehsils and villages
    // Consider implementing server-side search in production
    const allVillages = [];
    const tehsilCodes = Object.keys(tehsilToSroMap);

    for (const code of tehsilCodes) {
        const villages = await fetchVillages(code);
        allVillages.push(...villages.map(v => ({
            ...v,
            tehsilCode: code
        })));
    }
...
/**
 * Get SRO code from tehsil code
 * @param {string} tehsilCode - The tehsil code
 * @returns {string} - The corresponding SRO code
 */
export const getSroCodeFromTehsil = (tehsilCode) => {
    return tehsilToSroMap[tehsilCode] || tehsilCode;
};

/**
 * Format display address for a property
 * @param {string} districtName - District name
 * @param {string} tehsilName - Tehsil name
 * @param {string} villageName - Village name
 * @returns {string} - Formatted address
 */
export const formatLocationAddress = (districtName, tehsilName, villageName) => {
    const parts = [];
    if (villageName) parts.push(villageName);
    if (tehsilName) parts.push(tehsilName);
